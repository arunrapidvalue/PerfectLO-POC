            ...
            for each in sub_content:
                ...
                if settings.ANSWER_TYPE[str(answer_type)] == "check_box":
                    ...
                    if name or len(each['AnswerList'])>11:
                        ...
                        is_dropdown = True
                        strHTML += """<div class="row"><div class="col-md-3"><label class="form-check-label">%s</label></div>"""%(name)
                        if name:
                            id_name = name.replace(" ", "")
                        else:
                            id_name = str(sub_content_looper)
                        strHTML += """<div class="col-md-4"><select classs="form-control" style="width:50%%" name="dropdown_list" id="%s">"""%("id_"+id_name)
                ...
                for questions in each['AnswerList']:
                    ...
                    if prompt_text:
                        prompt_data = (questions["PromptText"]).replace(" ", "").replace("(","").replace(")","").replace(",","").replace("/","").replace("+","") 
                    ...
                    if is_check_box or is_radio:
                        ...
                        strHTML += raw_data%(value, element_id, prompt_data, answer_data, current_value, element_id, prompt_text)
